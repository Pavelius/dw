...
void scene::askencounter() const {
	logs::add(Attack, "Атаковать их.");
	logs::add(Hide, "Спрятаться, переждать и посмотреть что будет.");
	//if(reaction != Hostile) {
	//	logs::add(Retreat, "Повернуть назад и искать другой путь.");
	//	logs::add(Conversation, "Вступить в переговоры.");
	//	logs::add(Wait, "Ехать своей дорогой дальше не обращая внимание.");
	//}
}
