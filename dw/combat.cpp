...
void ask_spells(hero& player, monster& enemy) {
	for(auto i = FirstSpell; i <= LastSpell; i=(spell_s)(i+1)) {
		if(!isallow(player, enemy, i))
			continue;
		an.add(variant(i), "»спользовать заклинание '%1'", getstr(i));
	}
}
...
void hero::turnundead(monster& enemy) {
	...
	switch(result) {
	case Fail:
		enemy.act("%1 зарычал%а от €рости и бросил%ась в атаку.", enemy.getname(temp));
		sufferharm(enemy.getharm());
		break;
	case PartialSuccess:
...
static void description(monster& enemy) {
	...
	case Far:
		sb.add("ƒалеко впереди вы заметили %1.", enemy.getname(temp));
		break;
	case Near:
		sb.add("Ќедалеко от вас вы заметили %1.", enemy.getname(temp));
		break;
	default:
		sb.add("ќколо вас находитс€ %1.", enemy.getname(temp));
		break;
	}
}
