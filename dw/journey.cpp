...
void game::journey() {
	...
	switch(scout_result) {
	case Success:
		scout->act("%герой разведывал%а местность и сумел%а избежать встреч с непри€тност€ми.");
		break;
	case PartialSuccess:
		//combat(wander_monster, Far);
		break;
	case Fail:
		scout->act("» вдруг по дороге вы попали в засаду!");
	...
	case PartialSuccess:
		...
		break;
	case Fail:
		pathfinder->act("¬ы не сумели найти дорогу, поэтому заблудились и блукали на несколько дней больше.");
