...
void hero::supply(item* source, unsigned count) {
	while(true) {
		...
		for(unsigned i = 0; i < count; i++) {
			...
			if(cost > cup)
				continue;
			logs::add(tid(Actions, i), "%1. Цена [%2i] %3.", source[i].getname(temp, zendof(temp), true), cost, maptbl(text_golds, cost));
		}
		if(logs::getcount() <= 0) {
		...
		if(id.type == DungeonMoves) {
			...
			logs::add(" - Вы хотите купить %1 за [%2i] монет? - спросил владелец магазина.", it.getname(temp, zendof(temp), false), cost);
			if(logs::yesno()) {
				addcoins(-cost);
				pickup(it);
			}
