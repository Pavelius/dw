...
void hero::inflictharm(thing& enemy, int count) {
	...
	count -= armor;
	if(count <= 0) {
		enemy.act("Ѕрон€ полностью погасила удар.");
		return;
	}
	auto hp = enemy.gethp();
	hp -= count;
	...
	if(enemy.getcount() > 0) {
		...
		case 0: return;
		case 1: sb.adds("ќсталс€ еще [один]."); break;
		case 2: sb.adds("ќсталось еще [двое]."); break;
		case 3: sb.adds("ќсталось еще [трое]."); break;
		default: sb.adds("ќсталось еще [%1i].", enemy.getcount()); break;
		}
...
void hero::sufferharm(int count, bool ignore_armor) {
	...
	if(is(SpellDefense) && getongoing()) {
		...
		an.add(0, "Ќехочу убирать никаких заклинаний.");
