...
static void choose_homeland_traits(hero* player, bool interactive) {
	...
	for(auto e : ei.traits)
		an.add(e, getstr(e));
	an.sort();
	auto result = (trait_s)an.choose(interactive, true, "Как и о большинстве жителей [%1] о вас можно сказать что вы...", ei.nameof);
	player->set(result, player->get(result) + 1);
}
...
static void choose_homeland(hero* player, bool interactive) {
	...
	for(auto e : homeland_locations)
		an.add(e, getstr(e));
	an.sort();
	player->sethomeland((location_s)an.choosev(interactive, true, false, "Где вы родились?"));
}
