...
static void dwarven_tools(const featurei& info, creature& player, bool interactive) {
	player.apply(dwarven_tools_proficience, "Выбирайте [гномью] профессию", 1, interactive);
}

static void fighting_style(const featurei& info, creature& player, bool interactive) {
	player.apply(fighting_style_feats, "Выбирайте ваш стиль боя", 1, interactive);
}

static void divine_domain(const featurei& info, creature& player, bool interactive) {
	auto domain = player.choose_domain(interactive);
	player.set(domain);
}

static void cantrips(const featurei& info, creature& player, bool interactive) {
	player.apply(info.features, "Выберите Заговоры", player.getslots(0), interactive);
}

static void all(const featurei& info, creature& player, bool interactive) {
	for(auto f : info.features)
		player.set(f);
}

static featurei feature_data[] = {{Fighter, 1, "Боевой стиль", fighting_style},
