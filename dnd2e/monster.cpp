...
static custom_weapons dwarf_weapon[] = {{15, ShortSword, Crossbow},
...
static dungeon_encounter human_subtable[] = {{25, Bandit, {5, 15}},
{30, Bandit, {3, 9}},
{45, Bandit, {5, 15}},
{100, Character, {2, 5}},
};
...
} monster_data[] = {{"character", "персонаж", "персонажей", Common, {1}, 10, 12, {1, HD}, 0, 0, {{1, 2}}, Average, Neutral},
...
static char random(intellegence_s v) {
	switch(v) {
	case None: return 0;
	case AnimalInt: return 1;
	case Semi: return xrand(2, 4);
	case Low: return xrand(5, 7);
	default: return xrand(8, 10);
	case Very: return xrand(11, 12);
	case Hight:return xrand(13, 14);
	case Exeptional: return xrand(15, 16);
	case Genius: return xrand(17, 18);
	case SupraGenius: return xrand(19, 20);
	case Godlike: return xrand(21, 25);
	}
}
...
character::character(monster_s type) {
	...
	for(auto& c : abilities)
		c = 10;
	abilities[Intellegence] = random(e.intellegence);
	// Add custom weapon
	if(e.custom_weapon) {
